...
AddEventHandler('onResourceStart', function(resourceName)
	...
	for _,playerId in ipairs(players) do
		...
		MySQL.Async.fetchAll('SELECT status FROM users WHERE identifier = @identifier', {
			...
			TriggerClientEvent('life_status:load', playerId, data)
		end)
	end
end)
...
AddEventHandler('life_status:getStatus', function(playerId, statusName, cb)
	local xPlayer = ESX.GetPlayerFromId(playerId)
	local status  = xPlayer.get('status')
...
RegisterServerEvent('life_status:update')
AddEventHandler('life_status:update', function(status)
